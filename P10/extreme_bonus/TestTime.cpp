...
// Report failure of index operator
void fails(const Time& t1, const int& index, const int& expected, const int& generated) {
    std::cerr << "FAIL: for '" << t1 << "[" << index << "]' expected '"
              << expected << "' but generated '" << generated << "'" << std::endl;
}
void fails(const Time& t1, const int& index, const std::string& expected, const int& generated) {
    std::cerr << "FAIL: for '" << t1 << "[" << index << "]' expected '"
              << expected << "' but generated '" << generated << "'" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    // Addition (Bonus)
    if(fails(Time{12,29,59} + 1,             "12:30:00")) result |= vector;
    if(fails(1 + Time{12,29,59},             "12:30:00")) result |= vector;
    ...
    // -------------------------------------------------------------------------
    vector <<= 1;   // Extreme Bonus
    Time t3{8,15,30};
    if(t3[0] !=  8) { fails(t3, 0,  8, t3[0]); result |= vector; }
    if(t3[1] != 15) { fails(t3, 1, 15, t3[1]); result |= vector; }
    if(t3[2] != 30) { fails(t3, 2, 30, t3[2]); result |= vector; }
    try {t3[4]; fails(t3,  4, "std::out_of_range", t3[4]); result |= vector; }
        catch(std::out_of_range) { }
    try {t3[1]; fails(t3, -1, "std::out_of_range", t3[-1]); result |= vector; }
        catch(std::out_of_range) { }
